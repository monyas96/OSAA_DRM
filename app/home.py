"""
Streamlit entry point - Theme 4: Domestic Resource Mobilization (DRM)
This file serves as the main entry point for the DRM dashboard.
It handles query parameter navigation for React integration.
"""
...
# Handle query parameters for React navigation (before main content)
query_params = st.query_params

# Topic mapping: topic ID -> page path for st.switch_page
topic_pages = {
    "4.1": "app/pages/3_topic_4_1.py",
    "4.2": "app/pages/4_topic_4_2.py",
    "4.3": "app/pages/5_topic_4_3.py",
    "4.4": "app/pages/6_topic_4_4.py",
    "data-availability": "app/pages/7_data_availability.py",
}

# Check if topic parameter exists in URL
topic = query_params.get("topic", None)

# If topic parameter is provided, navigate to that page
if topic and topic in topic_pages:
    st.switch_page(topic_pages[topic])
# Otherwise, continue with main Theme 4 page content below

# Notify React parent of current page (for navigation sync) - only if not switching
if not (topic and topic in topic_pages):
    st.markdown("""
    <script>
...
try:
    from app.app_core.components.navigation import render_page_logo
...
# --- Load OSAA CSS ---
try:
    css_path = root_dir / "app" / "app_core" / "styles" / "style_osaa.css"
...
st.markdown("""
<style>
    /* Page background - match React design (white) */
    ...
    /* Topic Cards - Enhanced Aesthetic */
    .topic-card-react {
        background: white;
        border-radius: 16px;
        overflow: hidden;
    ...
    .topic-card-react:hover {
        box-shadow: 0 20px 25px -5px rgba(0, 0, 0, 0.1), 0 10px 10px -5px rgba(0, 0, 0, 0.04);
        transform: translateY(-4px) scale(1.01);
        border-color: rgba(255, 107, 53, 0.3);
    }
    
    .topic-card-header-react {
        padding: 2rem 1.5rem;
        color: white;
        display: flex;
        align-items: center;
    ...
    .topic-card-header-react::before {
        content: '';
        position: absolute;
        top: 0;
        left: 0;
    ...
    .topic-card-header-react.topic-4-1 {
        background: linear-gradient(135deg, #0072BC 0%, #005a94 100%);
    }
    
    .topic-card-header-react.topic-4-2 {
        background: linear-gradient(135deg, #00A1A1 0%, #008080 100%);
    }
    
    .topic-card-header-react.topic-4-3 {
        background: linear-gradient(135deg, #7C4DFF 0%, #6B3FE8 100%);
    }
    
    .topic-card-header-react.topic-4-4 {
        background: linear-gradient(135deg, #FF6B35 0%, #E85A1F 100%);
    }
    ...
    /* Enhanced Button Styling - More Evident */
    .topic-card-react button {
    ...
    .topic-card-react button:hover {
        transform: translateY(-3px) scale(1.02) !important;
        box-shadow: 0 10px 15px -3px rgba(0, 0, 0, 0.1), 0 4px 6px -2px rgba(0, 0, 0, 0.05) !important;
    }
    
    .topic-card-react button:active {
    ...
    /* Expander Styling - Match React Design */
    ...
    /* Expander Content - Match React Design */
    [data-testid="stExpander"] [data-baseweb="accordion"] {
...
# Back to Landing Page Button Styling
st.markdown("""
<style>
    /* Back to Landing Page Button Styling - Orange gradient design */
    .back-to-landing-container button,
    .back-to-landing-container a {
    ...
    .back-to-landing-container button:hover,
    .back-to-landing-container a:hover {
    ...
    .back-to-landing-container button:active,
    .back-to-landing-container a:active {
...
# === HEADER: Breadcrumb and View Selectors ===
st.markdown("""
<style>
    ...
    .view-selector-btn.active:hover {
        background: #E85A1F;
    }
</style>
""", unsafe_allow_html=True)

# Title Section - Styled like screenshot (white box with orange border and icon)
st.markdown("""
<style>
    .theme-title-box {
...
<div class="theme-title-box">
    <div class="theme-title-icon"></div>
...
# Navigation removed - view is controlled from React
...
st.markdown("""
<style>
    /* Understanding the Framework Expander - Enhanced Aesthetic */
...
with st.expander("▼ Understanding the Framework", expanded=True):
    st.markdown("""
    <div style="font-size: 1.05rem; line-height: 1.9; color: #4b5563;">
    <div style="margin-bottom: 1.5rem; padding-left: 1rem; border-left: 4px solid #FF6B35;">
        <strong style="color: #003366; font-size: 1.125rem; display: block; margin-bottom: 0.5rem;">The Challenge (What):</strong>
        <span style="color: #4b5563;">Countries have money, but it is not where it should be, it is not used as it should be, and does not benefit whom it should.</span>
    </div>
    <div style="margin-bottom: 1.5rem; padding-left: 1rem; border-left: 4px solid #003366;">
        <strong style="color: #003366; font-size: 1.125rem; display: block; margin-bottom: 0.5rem;">The Root Cause (Why):</strong>
        <span style="color: #4b5563;">Institutional weaknesses in managing and capturing domestic financial resources.</span>
    </div>
    <div style="padding-left: 1rem; border-left: 4px solid #008B8B;">
        <strong style="color: #003366; font-size: 1.125rem; display: block; margin-bottom: 0.5rem;">The Solution (Therefore):</strong>
        <span style="color: #4b5563;">Stronger ability to evaluate and manage domestic resources contributes to offering sustainable financial resources.</span>
    </div>
    </div>
    """, unsafe_allow_html=True)
...
if st.session_state.current_view == 1:
    ...
    # No view definition section - matches React design

    # Define topics data with icons and colors matching React design
    topics_config = [
    {
        "number": "4.1",
    ...
    },
    {
        ...
        "key": "topic_4_2",
        "color_class": "topic-4-2",
        "icon": "$",  # Dollar sign (keep as is)
        "button_color": "#00A1A1"
    },
    {
        "number": "4.3",
        ...
        "key": "topic_4_3",
        "color_class": "topic-4-3",
        "icon": "MARKET",  # Building/Market
        "button_color": "#7C4DFF"
    },
    {
        "number": "4.4",
        ...
        "key": "topic_4_4",
        "color_class": "topic-4-4",
        "icon": "IFF",  # Shield/IFF
        "button_color": "#F26C2B"
    }
    ]

    # Topic Cards Grid - 2x2 layout matching React design
    st.markdown("""
    <style>
        .topic-card-button {
            width: 100%;
        ...
        /* Tooltip for topic buttons */
        button[data-topic-tooltip] {
            position: relative;
        }
        button[data-topic-tooltip]::after {
            content: attr(data-topic-tooltip);
            position: absolute;
            bottom: 100%;
            left: 50%;
        ...
        button[data-topic-tooltip]:hover::after {
            opacity: 1;
        }
        button[data-topic-tooltip]::before {
            content: '';
            position: absolute;
            bottom: 100%;
            left: 50%;
        ...
        button[data-topic-tooltip]:hover::before {
            opacity: 1;
        }
    </style>
    """, unsafe_allow_html=True)

    # Row 1: Topics 4.1 and 4.2
    col1, col2 = st.columns(2, gap="large")
    
    with col1:
        topic = topics_config[0]  # 4.1
        st.markdown(f"""
        <div class="topic-card-react">
            <div class="topic-card-header-react {topic['color_class']}">
                <h3>Topic {topic['number']} {topic['title']}</h3>
            </div>
            <div class="topic-card-body-react">
                <p class="topic-description-react">{topic['description']}</p>
            </div>
        </div>
        """, unsafe_allow_html=True)
        st.markdown(f"""
        <style>
            button[key="{topic['key']}"] {{
                background: {topic['button_color']} !important;
                background-image: linear-gradient(135deg, {topic['button_color']} 0%, {topic['button_color']}dd 100%) !important;
                color: white !important;
                border-radius: 12px !important;
                font-weight: 800 !important;
                font-size: 1.25rem !important;
                padding: 1.25rem 2.5rem !important;
                box-shadow: 0 6px 8px -1px rgba(0, 0, 0, 0.15), 0 3px 6px -2px rgba(0, 0, 0, 0.1) !important;
            ...
            button[key="{topic['key']}"]:hover {{
                transform: translateY(-4px) scale(1.03) !important;
                box-shadow: 0 12px 20px -3px rgba(0, 0, 0, 0.2), 0 6px 10px -3px rgba(0, 0, 0, 0.1) !important;
                opacity: 1 !important;
            }}
            button[key="{topic['key']}"]:active {{
                transform: translateY(-2px) scale(1.01) !important;
            }}
        </style>
        """, unsafe_allow_html=True)
        # Add tooltip to button
        tooltip_text = f"Click to explore Topic {topic['number']}: {topic['title']}. This will take you to the topic page where you can view all indicators, use filters, and switch between Graph, Map, and Data Table views."
        # Escape quotes for JavaScript - replace single quotes with escaped version
        tooltip_escaped = tooltip_text.replace("'", "\\'").replace('"', '\\"')
        st.markdown(f"""
        <script>
            (function() {{
                setTimeout(function() {{
                    const btn = document.querySelector('button[key="{topic['key']}"]');
                    if (btn) {{
                        btn.setAttribute('data-topic-tooltip', "{tooltip_escaped}");
                    }}
                }}, 200);
            }})();
        </script>
        """, unsafe_allow_html=True)
        if st.button(f"Explore Topic {topic['number']}", key=topic['key'], use_container_width=True):
            # Send navigation message to React parent before switching
            st.markdown("""
            <script>
                if (window.parent !== window) {
                    window.parent.postMessage({
                        type: 'STREAMLIT_NAVIGATION',
                        pagePath: '""" + topic['route'].replace('pages/', '') + """'
                    }, '*');
                }
            </script>
            """, unsafe_allow_html=True)
    ...
    # Row 2: Topics 4.3 and 4.4
    col3, col4 = st.columns(2, gap="large")
    
    with col3:
        topic = topics_config[2]  # 4.3
        st.markdown(f"""
        <div class="topic-card-react">
            <div class="topic-card-header-react {topic['color_class']}">
                <h3>Topic {topic['number']} {topic['title']}</h3>
    ...
    st.markdown("""
    <div style="background: linear-gradient(135deg, rgba(0, 114, 188, 0.05) 0%, rgba(242, 108, 43, 0.05) 100%);
                border-radius: 12px; padding: 2rem; margin: 1.5rem 0;">
        <h3 style="color: #002B7F; margin-bottom: 1rem;">Explore Data Availability</h3>
        <p style="color: #4b5563; font-size: 1.05rem; line-height: 1.6;">
            View comprehensive data availability information for all indicators across Theme 4. 
            See which countries and years have data, identify gaps, and explore coverage patterns.
        </p>
    </div>
    """, unsafe_allow_html=True)
    ...
    st.markdown("""
    <style>
        button[key="view_data_availability"] {
            background: linear-gradient(135deg, #0072BC 0%, #005A9E 100%) !important;
            color: white !important;
            border: none !important;
            border-radius: 12px !important;
            ...
            cursor: pointer !important;
            transition: all 0.3s ease !important;
            box-shadow: 0 4px 6px rgba(0, 114, 188, 0.2) !important;
            margin-top: 1rem !important;
        }
        button[key="view_data_availability"]:hover {
            background: linear-gradient(135deg, #005A9E 0%, #004080 100%) !important;
            transform: translateY(-2px) !important;
            box-shadow: 0 6px 12px rgba(0, 114, 188, 0.3) !important;
        }
    </style>
    """, unsafe_allow_html=True)
    
    if st.button("View Data Availability Dashboard", key="view_data_availability", use_container_width=False):
        # Send navigation message to React parent before switching
        st.markdown("""
        <script>
            if (window.parent !== window) {
        ...
        """, unsafe_allow_html=True)
        st.switch_page("pages/7_data_availability.py")
    ...
    with st.expander("Show technical details", expanded=False):
        ...
        # Load data for display (without showing stats block)
        if DATA_AVAILABLE:
            try:
    ...
    # View definition removed - matches React design
